...
import '../data/local_magazine_store.dart';
...
class MagazineScreen extends StatefulWidget {
  const MagazineScreen({this.storageScope = 'preview', super.key});
...
class _MagazineScreenState extends State<MagazineScreen> {
  ...
  late final LocalMagazineStore _magazineStore = LocalMagazineStore(
    'local_magazine_${widget.storageScope}_v1',
  );
  ...
  Future<void> _restorePhotos() async {
    final paths = await _magazineStore.loadPhotoPaths();
    final existing = <XFile>[];
    for (final path in paths) {
      if (await File(path).exists()) {
        existing.add(XFile(path));
      }
    }
    if (!mounted) return;
    setState(() {
      _photos
        ..clear()
        ..addAll(existing);
      _showPreview = existing.isNotEmpty;
    });
  }
  ...
  Future<void> _pickImages() async {
    ...
    setState(() {
      for (final image in images) {
        if (!_photos.any((photo) => photo.path == image.path)) {
          _photos.add(image);
        }
      }
      _showPreview = false;
  ...
  Future<void> _removePhoto(int index) async {
    ...
    setState(() {
      ...
      _showPreview = _photos.isNotEmpty;
    });
    await _magazineStore.savePhotoPaths(_photos.map((photo) => photo.path));
  }

  Future<void> _createMagazine() async {
    if (_photos.isEmpty) {
      showAppSnackBar(context, '매거진에 담을 사진을 한 장 이상 추가해 주세요.');
      return;
    }
    if (_period == _PeriodOption.custom && _customRange == null) {
      showAppSnackBar(context, '먼저 기간을 선택해 주세요.');
      return;
    }
    ...
    try {
      _selectedRange();
      final persisted = <XFile>[];
      for (final photo in _photos) {
        final path = await _mediaStore.persist(
          sourcePath: photo.path,
          kind: MissionMediaKind.photo,
        );
        persisted.add(XFile(path));
      }
      await _magazineStore.savePhotoPaths(persisted.map((photo) => photo.path));
      ...
      showAppSnackBar(context, '최적화한 사진을 이 기기에 저장했어요.');
    } catch (_) {
      if (mounted) _showSaveError('사진을 이 기기에 저장하지 못했습니다.');
    } finally {
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const Text(
              'AI가 사진의 분위기와 흐름을 읽어 감도 있는 이야기를 엮어드려요.',
              style: TextStyle(color: Color(0xFF6D6875)),
            ),
            ...
            Text(
              '${_photos.length}장의 사진이 선택되었어요',
              style: const TextStyle(color: Color(0xFF6D6875)),
            ),
            ...
            FilledButton.icon(
              key: const Key('createMagazineButton'),
              onPressed: _isCreating ? null : _createMagazine,
              style: FilledButton.styleFrom(
                minimumSize: const Size.fromHeight(56),
                backgroundColor: const Color(0xFF6750E8),
              ),
              icon: _isCreating
                  ? const SizedBox(
                      width: 19,
              ...
              label: Text(_isCreating ? '사진을 최적화하고 있어요...' : '로컬 매거진 만들기'),
            ),
            if (_showPreview) ...[
              const SizedBox(height: 28),
              _MagazinePreview(photos: _photos, period: _periodLabel),
            ],
          ],
        ),
      ),
  ...
  String _optionText(_PeriodOption option) => switch (option) {
    _PeriodOption.day => '하루동안',
    _PeriodOption.week => '일주일동안',
    _PeriodOption.month => '한달동안',
    _PeriodOption.custom => '직접 선택',
  };
}
    ...
    child: InkWell(
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
            Icon(icon, color: const Color(0xFF6750E8)),
            const SizedBox(height: 7),
      ...
      Container(
        ...
        child: Stack(
          ...
          children: [
            Opacity(
              opacity: .56,
              child: Image.file(File(photos.first.path), fit: BoxFit.cover),
            ),
            DecoratedBox(
              decoration: BoxDecoration(
            ...
            Positioned(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${photos.length} photos · AI editorial',
                    style: const TextStyle(
